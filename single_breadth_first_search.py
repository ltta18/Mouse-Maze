...
def single_bfs(maze_size, wall_positions, prize_list, start_position):
    """
    ...
    frontier: queue - stores nodes to expand since we need to expand the oldest node every time we loop
    """
    ...
    frontier = deque()
    frontier.append(StateRep(start_position, [], prize_list)) 

    while(len(frontier)!=0):
        #expand the oldest node added to the frontier & increase the number of expanded nodes by 1
...
def print_solution(maze_filename, maze_output):
    ...
    solution = single_bfs(maze_size, wall_positions, prize_list, start_position)
...
if __name__ == "__main__":
    print_solution('1prize-medium.txt','bfs_maze_output-medium.txt')
    print_solution('1prize-large.txt','bfs_maze_output-large.txt')
    print_solution('1prize-open.txt','bfs_maze_output-open.txt')